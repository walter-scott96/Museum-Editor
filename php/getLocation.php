...
<?php
    ...
    if(isset($_GET['name']))
    {
        ...
        $locationResult = mysqli_query($link, $query);

        $query = "SELECT * FROM `image` WHERE `lname` = '$name'";
        $imageResult = mysqli_query($link, $query);

        $query = "SELECT * FROM `related` WHERE `lname` = '$name'";
        $relatedResult = mysqli_query($link, $query);
        
        $tables = array($locationResult, $imageResult, $relatedResult);
        $data = array();
